...
app.use(session({secret:process.env.SESSION_SECRET||'dev-only-change-me',resave:false,saveUninitialized:true,cookie:{httpOnly:true,sameSite:'lax',secure:false}}));
app.use(attachUser);
...
app.get('/auth/roblox/callback',async(req,res,next)=>{try{const profile=await callback(req);const robloxId=String(profile.sub);let user=db.prepare('SELECT * FROM users WHERE roblox_id=?').get(robloxId); if(user){db.prepare('UPDATE users SET username=?, display_name=?, updated_at=CURRENT_TIMESTAMP WHERE id=?').run(profile.preferred_username||profile.name||robloxId,profile.name||'',user.id); req.session.userId=user.id;} else {const owner=process.env.INITIAL_OWNER_ROBLOX_ID&&robloxId===String(process.env.INITIAL_OWNER_ROBLOX_ID); const info=db.prepare('INSERT INTO users(roblox_id,username,display_name,role,is_allowed) VALUES(?,?,?,?,?)').run(robloxId,profile.preferred_username||profile.name||robloxId,profile.name||'',owner?'owner':'member',owner?1:0); req.session.userId=info.lastInsertRowid;} res.redirect('/dashboard');}catch(e){next(e);}});
app.post('/logout',(req,res)=>req.session.destroy(()=>res.redirect('/')));
app.get('/dashboard',requireLogin,(req,res)=>{const stats={firefighters:db.prepare('SELECT COUNT(*) c FROM firefighters').get().c,cases:db.prepare('SELECT COUNT(*) c FROM disciplinary_actions').get().c,pst:db.prepare("SELECT COUNT(*) c FROM users WHERE role IN ('owner','pst')").get().c}; const recent=db.prepare('SELECT a.*, u.username FROM audit_log a LEFT JOIN users u ON u.id=a.actor_user_id ORDER BY a.id DESC LIMIT 8').all(); res.render('dashboard',{title:'Dashboard',stats,recent});});
